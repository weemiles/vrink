...
## VRINK Project Overrides

This project uses the Apple-inspired structure, spacing, product-first imagery, and light/dark section rhythm as the visual reference. For the VRINK homepage, these rules override the default Apple typography and copy assumptions:

- Use Korean for all user-facing homepage copy, navigation labels, calls to action, and section headings.
- Use `Pretendard` for every text surface: body, headings, buttons, navigation, captions, labels, and numeric text.
- Do not use the excluded technical abbreviation in homepage copy, section headings, calls to action, metadata, or content files. Use "스마트", "개인 맞춤", "데이터 기반", "자동 운영", and "공간 맞춤 음료 시스템" instead.
- Product imagery from `public/images/vrink/apple/` is the primary visual system for the homepage.
- Typography scale follows the DJI Figma reference while keeping Pretendard as the only font family. Use compact navigation and CTA text around 13px, labels around 14px, Korean body copy around 15px, supporting copy around 15.6px, card titles around 20px, feature titles around 27px, section headings around 32px, and page display headings around 36px.
- Keep weights restrained: regular for navigation, CTAs, labels, and body copy; medium for product subtitles and card headings; semibold only for major section or feature headings.
- Do not apply Figma's negative tracking. Korean text keeps `letter-spacing: 0` across the site.
