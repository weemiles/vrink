/**
 * KeppinLogo — keppin 브랜드 커스텀 로고 마크
 *
 * 디자인 컨셉: "Connection K"
 * - 세로 줄기(Stem): 나 자신, 중심축
 * - 두 개의 곡선 팔(Arms): 관계의 연결선
 * - 끝단 노드(Dots): 연결된 사람들
 * - 전체 형태가 "K"(keppin)를 이루면서 동시에
...
interface KeppinLogoProps {
  size?: number;
  className?: string;
}

export function KeppinLogo({ size = 32, className }: KeppinLogoProps) {
  return (
    <svg
      width={size}
      height={size}
...
/**
 * KeppinAppIcon — 라운드 스퀘어 안에 로고 마크가 들어간 앱 아이콘
 *
 * variant:
 *  'blue'  — 파란 배경 + 흰 마크 (로그인, 회원가입 등)
 *  'white' — 흰 배경 + 파란 마크 (스플래시 등)
 */
interface KeppinAppIconProps {
  size?: number;
  variant?: 'blue' | 'white';
  borderRadius?: number;
  className?: string;
...
export function KeppinAppIcon({
  ...
  return (
    ...
    >
      <KeppinLogo size={markSize} className={markClass} />
    </div>
  );
}